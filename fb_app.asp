...
'' in FACEBOOK_SECRET or this example will not run correctly
'' 
'' This is the error you will get from Facebook if the 
'' To fix this error, please set your Connect URL in the application settings 
'' editor. Once it has been set, users will be redirected to that URL instead 
'' of this page after logging in......
''

dim FACEBOOK_APP_ID: FACEBOOK_APP_ID = ""
dim FACEBOOK_SECRET: FACEBOOK_SECRET = ""
...
function get_facebook_cookie(app_id, application_secret) 
  ...
  sCookie = request.cookies("fbs_" & FACEBOOK_APP_ID)
  ' URL Decode the cookie
  sCookie = URLDecode( sCookie )
  ' Get rid of erroneous quotes  
  sCookie = replace( sCookie, chr(34), "" )
  
  
  if sCookie = "" then '' Not Logged In
    response.write ""
  else
      '' Split the cookie by ambersand
      arCookie = split( sCookie, "&" )
      for iX = 0 to ubound( arCookie ) 
        sItm = arCookie( iX )
        if instr(sItm, "=")>0 then 
            arItm = split( sItm, "=" )
            args.add trim( arItm(0) ), trim( arItm(1) )
        end if
      next
      
      '' Now sort the list of arguments
      ksort(args)
      ...
      for each key in args 
        if key <> "sig" then
          payload = payload & key & "=" & args( key )
        end if
      next 
     
      '
      ' Make sure the payload string + application secret 
      ' matches the signature passed in the cookie
      ' 
      if enc.md5(payload & application_secret) <> args("sig")  then
...
function ksort(byref objDict )
    SortDictionary objDict, 1 
end function
...
'***************************************************************************
function get_page_contents(strURL)
   Dim objHTTP
   dim strData
